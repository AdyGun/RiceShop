<?php
	...
	if (isset($_POST)){
		...
		if ($_POST['hidden']['command']=="create" || $_POST['hidden']['command']=="update"){
			...
			if (strlen($fname)<4){
				...
				if ($_POST['hidden']['command']=='update'){
					$fhidname = $mysqli->real_escape_string(ucwords(strtolower($_POST['hidden']['name'])));
			...
			if ($go == true){
				if ($_POST['hidden']['command']=='create'){
					...
					if ($result = $mysqli->query($query)){
						...
						// Insert Activity Log
						addLog($mysqli,$flogin['user_id'],'Master Level User',$fid,'Tambah');
					}
					else{
						$alert[] = array(
							'type' => 'danger',
					...
					if ($result = $mysqli->query($query)){
						...
						// Insert Activity Log
						addLog($mysqli,$flogin['user_id'],'Master Level User',$fid,'Ubah');
						// Delete Level Access
		...
		if ($_POST['hidden']['command']=='delete'){
			$query = "DELETE FROM tuser_level WHERE level_id='$fid'";
			if ($result = $mysqli->query($query)){
				$alert[] = array(
					'type' => 'success',
				...
				// Insert Activity Log
				addLog($mysqli,$flogin['user_id'],'Master Level User',$fid,'Hapus');
				// Delete Level Access
				$dQuery = "DELETE FROM tlevel_access WHERE level_id = '$fid'";
				if (!$dResult = $mysqli->query($dQuery)){
					$alert[] = array(
